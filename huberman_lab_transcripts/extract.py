...
def _build_chapter(chapter_json: dict) -> PodcastChapter:
    return PodcastChapter(start_time=int(chapter_json.get('start_time')),
...
def read_podcast_info_file(info_file: Path) -> PodcastInfo:
    ...
    chapters = [_build_chapter(j) for j in raw_chapters]
    ...
    return PodcastInfo(video_id = info_json.get('id'),
...
class PodcastExtractor:
    ...
    def extract_all(self) -> List[PodcastInfo]:
        return [self.extract_info(video_id) for video_id in self.video_id_file_map.keys()]
